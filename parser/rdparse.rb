...
class Rule
  ...
  def initialize(name, parser)
    ...
    @parser = parser
    @matches = [] # Left-recursive matches
    @lrmatches = []
  end
    ...
    matches.each do |match|
      ...
      match.pattern.each_with_index do |token, index|
        ...
        if @parser.rules[token]
          ...
          @logger.debug("Matched '#{@name} = #{match.pattern[index..].inspect}'")
        else
      ...
      if result
        ...
        unless match_result.nil?
          @logger.debug("'#{@parser.string[start..@parser.pos - 1]}' matched '#{@name}' and generated: #{match_result}")
        end
        break
      else
